...
{
#pragma warning disable CS0246 // The type or namespace name 'Path' could not be found (are you missing a using directive or an assembly reference?)
    [CustomEditor(typeof(Path))]
#pragma warning restore CS0246 // The type or namespace name 'Path' could not be found (are you missing a using directive or an assembly reference?)
        ...
        public void OnSceneGUI()
        {
			for(int i =0; i < _path.Waypoints.Count; i++)
			{
                ...
                _path.Waypoints[i].position = Handles.FreeMoveHandle(
                    ...
                    HandleUtility.GetHandleSize(_path.Waypoints[i].position) * 0.1f, Vector3.one, Handles.CubeHandleCap);
				
				if (_path.IsCurved)
				{
                    Handles.color = Color.black;
					_path.SetHandle1(i, Handles.FreeMoveHandle(_path.GetHandle1(i), Quaternion.identity,
						HandleUtility.GetHandleSize(_path.GetHandle1(i)) * 0.1f, Vector3.one, Handles.CircleHandleCap));
