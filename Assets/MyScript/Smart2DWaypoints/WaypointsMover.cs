...
namespace Assets.Plugins.Smart2DWaypoints.Scripts
{
	public class WaypointsMover : MonoBehaviour
	{
		...
		private bool telep = false;
		private int idx;
		private float _pauseTime;
		private Vector3 nearpoint;
		private Transform nearway;
		private List<Vector3> pOints;
		private List<Transform> ways;
		private Path pat;
		...
		private void MoveTargetIndexTox(int idx)
		{
			_targetIndex = idx;
			_previousTargetIndex = _targetIndex;
			int dInd = _isForwardDirection ? 1 : -1;
			idx = (_targetIndex + dInd + Path.WaypointsCount) % Path.WaypointsCount;
			_passedWaypointsCount++;
		}
		...
		public IEnumerator OnWaypointReached()
	    {
		...
		public IEnumerator Teleports()
		{
			LayerMask mask = LayerMask.GetMask("Path");
			LayerMask mask1 = LayerMask.GetMask("waypoint");
			Collider2D colliders = Physics2D.OverlapCircle(transform.position, 0.15f, mask);
			pat = colliders.gameObject.GetComponent<Path>();
			pOints = pat.GetPoints();
			nearpoint = Waypoint.FindClosestpoint(transform.position, pOints);
			idx = pOints.IndexOf(nearpoint);
			ways = pat.Waypoints;
			nearway = Waypoint.FindClosestway(transform.position, ways);
			RaycastHit2D[] nextway = Physics2D.CircleCastAll(transform.position, 0.15f, transform.position, 50, mask1);
			telep = false;
			Go(pat, nearway);
			tele = false;
			yield return null;
